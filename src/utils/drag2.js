class Drag {
  ...
  init() {
    ...
    this.limit.addEventListener('click', this.click.bind(this), false)
  }
  getPosition() {
  ...
  destory() {
    this.limit.removeEventListener('click', this.click.bind(this))
  }
  getPercent(e) {
    let distance = (this.isVertical ? e.clientY : e.clientX) - this.limit_position

    distance = distance > this.limit_size ? this.limit_size : distance

    distance = distance < 0 ? 0 : distance

    let limit_size = this.limit_size

    this.per = this.isVertical ? 1 - distance / limit_size : distance / limit_size
  }
}
export default Drag
